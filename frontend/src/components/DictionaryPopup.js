...
const DictionaryPopup = ({ data, onClose, position }) => {
  ...
  // --- LẤY NGHĨA TIẾNG VIỆT (Mới thêm) ---
  // Nếu Backend chưa kịp trả về thì để trống
  ...
  const handleSaveFlashcard = async () => {
    try {
      ...
      // Ưu tiên lưu nghĩa Tiếng Việt vào mặt sau thẻ
      // Nếu không có tiếng Việt thì mới dùng tiếng Anh
      const backContent = vietnamese ? vietnamese : firstDefinition;

      const response = await fetch('https://khkt-1-7l7b.onrender.com/api/flashcards', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          front: word,
          back: backContent, // Lưu nghĩa tiếng Việt vào đây
